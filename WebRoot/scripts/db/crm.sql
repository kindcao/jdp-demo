...
-- ----------------------------
CREATE ALGORITHM=UNDEFINED DEFINER=`user`@`%` SQL SECURITY DEFINER VIEW `market_event_view_sub` AS select `me`.`id` AS `id`,`me`.`occur_date` AS `occur_date`,date_format(`me`.`occur_date`,'%Y-%m-%d') AS `occur_date_str`,`me`.`begin_time` AS `begin_time`,time_format(`me`.`begin_time`,'%i:%s') AS `begin_time_str`,`me`.`end_time` AS `end_time`,time_format(`me`.`end_time`,'%i:%s') AS `end_time_str`,`me`.`subject` AS `subject`,`met`.`superior_id` AS `mktevt_superior_id`,`met`.`id` AS `mktevt_id`,`met`.`name` AS `mktevt_name` from (`market_event` `me` join `market_event_type` `met`) where (`me`.`market_event_type_id` = `met`.`id`);
