/*
 * Ext JS Library 1.0 Beta 1 Copyright(c) 2006-2007, Ext JS, LLC.
 * licensing@extjs.com
 * 
...
Element.collectTextNodes = function(element) {
	return $A($(element).childNodes).collect(function(node) {
		return (node.nodeType == 3 ? node.nodeValue : (node.hasChildNodes()
...
Element.collectTextNodesIgnoreClass = function(element, className) {
	return $A($(element).childNodes).collect(function(node) {
		return (node.nodeType == 3
				? node.nodeValue
				: ((node.hasChildNodes() && !Element.hasClassName(node,
...
Object.extend(Object.extend(Effect.Move.prototype, Effect.Base.prototype), {
			...
			setup : function() {
				...
				// ==> Always set top and left for position relative elements in
				// your stylesheets
			...
			update : function(position) {
				this.element.setStyle({
							left : Math.round(this.options.x * position
									+ this.originalLeft)
									+ 'px',
							top : Math.round(this.options.y * position
									+ this.originalTop)
									+ 'px'
						});
			}
...
Object.extend(Object.extend(Effect.Highlight.prototype, Effect.Base.prototype),
		{
			...
			setup : function() {
				...
				this._base = $R(0, 2).map(function(i) {
					return parseInt(this.options.startcolor.slice(i * 2 + 1, i
											* 2 + 3), 16)
				}.bind(this));
				this._delta = $R(0, 2).map(function(i) {
					return parseInt(this.options.endcolor.slice(i * 2 + 1, i
											* 2 + 3), 16)
							- this._base[i]
				}.bind(this));
			},
	...
	return new Effect.Scale(element, 100, Object.extend({
						...
						afterUpdateInternal : function(effect) {
							effect.element.down().setStyle({
								bottom : (effect.dims[0] - effect.element.clientHeight)
										+ 'px'
							});
						},
						afterFinishInternal : function(effect) {
...
Effect.Shrink = function(element) {
	...
	return new Effect.Parallel([new Effect.Opacity(element, {
					...
					new Effect.Scale(element, window.opera ? 1 : 0, {
						...
						afterFinishInternal : function(effect) {
							effect.effects[0].element.hide().undoClipping()
...
Object.extend(Object.extend(Effect.Morph.prototype, Effect.Base.prototype), {
	...
	setup : function() {
		...
		this.transforms = this.style.map(function(pair) {
			...
			if (value.parseColor('#zzzzzz') != '#zzzzzz') {
				...
				var components = value.match(/^([\+\-]?[0-9\.]+)(.*)$/), value = parseFloat(components[1]), unit = (components.length == 3)
						? components[2]
						: null;
	...
	update : function(position) {
		...
		this.transforms.each(function(transform) {
			value = transform.unit == 'color' ? $R(0, 2).inject('#',
					function(m, v, i) {
						return m
								+ (Math
					...
					: transform.originalValue
							+ Math
									.round(	((transform.targetValue - transform.originalValue) * position)
													* 1000) / 1000
							+ transform.unit;
...
Object.extend(Effect.Transform.prototype, {
			...
			play : function() {
				return new Effect.Parallel(this.tracks.map(function(track) {
							var elements = [$(track.ids) || $$(track.ids)]
									.flatten();
							return elements.map(function(e) {
										return new track.effect(e, Object
														.extend({
																	sync : true
																},
																track.options))
