...
(function() {
	...
	var B = navigator.userAgent.toLowerCase(), D = (B.indexOf("opera") > -1), K = (B
			.indexOf("safari") > -1), A = (!D && !K && B.indexOf("gecko") > -1), F = (!D && B
			.indexOf("msie") > -1);
	var E = {
		HYPHEN : /(-[a-z])/i
	};
	...
	YAHOO.util.Dom = {
		...
		getXY : function(M) {
			var N = function(P) {
				if (P.parentNode === null || P.offsetParent === null
						|| this.getStyle(P, "display") == "none") {
					return false
				}
				var O = null;
				var U = [];
				...
				if (P.getBoundingClientRect) {
					...
					var R = Math.max(S.documentElement.scrollTop,
							S.body.scrollTop);
					var T = Math.max(S.documentElement.scrollLeft,
							S.body.scrollLeft);
		...
		setXY : function(M, P, O) {
			var N = function(S) {
				...
				var T = [parseInt(this.getStyle(S, "left"), 10),
						parseInt(this.getStyle(S, "top"), 10)];
				if (isNaN(T[0])) {
	...
	YAHOO.util.Event = function() {
		...
		return {
			...
			_isValidCollection : function(K) {
				return (K && K.length && typeof K != "string" && !K.tagName
						&& !K.alert && typeof K[0] != "undefined")
			},
			elCache : {},
			getEl : function(K) {
			...
			_tryPreloadAttach : function() {
				...
				for (var L = 0, K = E.length; L < K; ++L) {
					...
					if (O) {
						...
						if (N) {
							if (!O.checkReady || H || N.nextSibling
...
YAHOO.util.Connect = {
	...
	handleTransactionResponse : function(E, F, A) {
		...
		if (C >= 200 && C < 300) {
			...
			switch (C) {
				...
				case 12152 :
				case 13030 :
					B = this.createExceptionObject(E.tId, F.argument, (A
									? A
									: false));
					if (F.failure) {
						if (!F.scope) {
						...
						for (var C = 0; C < A.options.length; C++) {
							if (A.options[C].selected) {
								if (window.ActiveXObject) {
									this._sFormData += encodeURIComponent(H)
											+ "="
											+ encodeURIComponent(A.options[C].attributes["value"].specified
													? A.options[C].value
													: A.options[C].text) + "&"
								} else {
									this._sFormData += encodeURIComponent(H)
	...
	createFrame : function(A) {
		...
		if (window.ActiveXObject) {
			var C = document.createElement("<iframe id=\"" + B + "\" name=\""
					+ B + "\" />");
			if (typeof A == "boolean") {
	...
	uploadFile : function(A, I, C, B) {
		...
		var D = function() {
			...
			try {
				L.responseText = H.contentWindow.document.body
						? H.contentWindow.document.body.innerHTML
						: null;
				L.responseXML = H.contentWindow.document.XMLDocument
						? H.contentWindow.document.XMLDocument
...
(function() {
	...
	B.doMethod = function(F, J, G) {
		var I = null;
		if (this.patterns.points.test(F)) {
			var H = this.method(this.currentFrame, 0, 100, this.totalFrames)
					/ 100;
